...
/// Official Clash Royale API card icon CDN URLs (`iconUrls.medium`).
/// Display-only via AsyncImage — not a bulk asset redistributor.
enum ClashRoyaleIconCatalog {
    static let mediumByName: [String: String] = [
    ...
    static var allCardNames: [String] { Array(mediumByName.keys).sorted() }

    static func mediumURL(forCardNamed name: String) -> URL? {
        guard let raw = mediumByName[name] else { return nil }
        return URL(string: raw)
    }

    static func mediumURLString(forCardNamed name: String) -> String? {
        mediumByName[name]
    }
}
